...
# You're in the middle of a circular lake.
# A goblin chases you around its circumference.
# If you reach the shore and the goblin isn't there, you escape.
...
def redraw(draw_text=False, win=False):
    ...
    pygame.draw.circle(window, (255, 0, 0),
                       (int(width / 2 + radius * math.cos(goblin)), int(height / 2 + radius * math.sin(goblin))), 6, 0)
...
while True:
    ...
    if clicking:
        game_x, game_y = pygame.mouse.get_pos()
        move_boat(game_x - width / 2, game_y - height / 2)
    update_goblin()
    detect_win()
    redraw()
    clock.tick(60)
